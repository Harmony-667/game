        ...
        when(options("How am I doing so far?", "Who are you?", "Where are you?", "Nothing really.")) {
            FIRST_OPTION -> {
                ...
                if(player.getSlayerAssignment() == null) {
                    chatNpc("You need something new to hunt. Come and see me", "when you can and I'll give you a new task.", npc = master.id)
                } else {
                    chatNpc("You're currently assigned to kill ${player.getSlayerAssignment()!!.identifier.lowercase()}; only ${player.attr[SLAYER_AMOUNT]}", "more to go.", npc = master.id)
                }
            }
            SECOND_OPTION -> {
